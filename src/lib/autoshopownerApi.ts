...
export type MobileServiceFields = {
  enabled?: boolean;
  coverageDistanceKm?: number | null;
};

export type MobileServiceData = {
  enabled: boolean;
  coverageDistanceKm: number | null;
};

/** PATCH /api/autoshopowner/profile/business/mobile-service — partial body supported. */
export function updateMobileService(token: string, fields: MobileServiceFields) {
  const body: Record<string, unknown> = {};
  if (fields.enabled !== undefined) body.enabled = fields.enabled;
  if (fields.coverageDistanceKm !== undefined) body.coverageDistanceKm = fields.coverageDistanceKm;
  return patchJsonAutoshopowner<ApiEnvelope & { data?: MobileServiceData }>(
    "/api/autoshopowner/profile/business/mobile-service",
    body,
    token,
  );
}
