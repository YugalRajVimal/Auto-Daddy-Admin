...
export type DigiPurseCategoryId = "ownership" | "pink-slip" | "driving-license" | "other-documents";

export const DIGI_PURSE_CATEGORIES: ReadonlyArray<{ id: DigiPurseCategoryId; label: string }> = [
  { id: "ownership", label: "Ownership" },
  { id: "pink-slip", label: "Pink Slip" },
...
export function fieldsForCategory(category: DigiPurseCategoryId): VehicleDocumentFieldKey[] {
  switch (category) {
    case "ownership":
      return ["carOwnershipCertificate"];
    case "pink-slip":
...
export function fieldLabelForCategory(category: DigiPurseCategoryId, key: VehicleDocumentFieldKey): string {
  if (category === "driving-license") {
    if (key === "drivingLicenseFront") return "Front";
    if (key === "drivingLicenseBack") return "Back";
  }
  return VEHICLE_DOCUMENT_FIELDS.find((f) => f.key === key)?.label ?? key;
}
...
function normalizeVehicle(raw: unknown): VehicleDocumentsVehicle | null {
  ...
  return {
    ...
    odometerReading:
      typeof x.odometerReading === "string" || typeof x.odometerReading === "number"
        ? x.odometerReading
        : null,
...
export function normalizeVehicleDocumentsList(
  payload: UserDocumentsApiResponse | null | undefined
): VehicleDocumentsRecord[] {
