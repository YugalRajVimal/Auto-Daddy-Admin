...
export default function OwnerInvoicesPage() {
  ...
  const detailCached =
    previewInvoice
      ? findJobCardById(previewInvoice.id)
      : jobCardViewerId
        ? findJobCardById(jobCardViewerId)
        : null;
      ...
      <div className="flex flex-col gap-4">
        {previewInvoice ? (
          <OwnerInvoiceEstimateView
            ...
            token={token}
            cachedJobCard={detailCached}
            invoiceNoHint={invoiceNoDisplay(previewInvoice)}
          ...
          <OwnerInvoiceEstimateView
            key={`jobcard-${jobCardViewerId}`}
            jobCardId={jobCardViewerId}
            token={token}
            cachedJobCard={detailCached}
            jobNoHint={
              detailCached
                ? resolveJobCardNo(detailCached)
                : invoiceRows.find((r) => r.id === jobCardViewerId)?.jobNo ?? null
            }
            callingCode={countryCode}
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-3">
                {vehicles.length > 0 ? (
                  <div className="flex flex-wrap items-end justify-end gap-3">
                    <div className="min-w-[11rem] w-full sm:w-auto sm:min-w-[14rem] sm:max-w-[16rem]">
                      <label className={ownerVehicleLabelClass} htmlFor="invoices-vehicle-filter">
                        Vehicle
                      </label>
                      <select
                        id="invoices-vehicle-filter"
                        value={vehicleFilter}
                        onChange={(e) => {
                ...
                {invoiceRows.length === 0 ? (
                  <EmptyState>
                    {vehicleFilter ? "No invoices for this vehicle." : "No invoices right now."}
                  </EmptyState>
                ) : (
                            ...
                            <th className={`${OWNER_TABLE_HEAD_TH_CLASS} w-10`}>
                              <input
                                ...
                                checked={
                                  ...
                                  selectedIds.length === invoiceRows.length
                                }
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setSelectedIds(invoiceRows.map((r) => r.id));
                                  } else setSelectedIds([]);
                                }}
                          ...
                          {invoiceRows.map((row, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td
                                  className={`${OWNER_TABLE_BODY_TD_CLASS} font-semibold text-slate-600`}
                                >
                                  $ 0
                                </td>
                                <td className={OWNER_TABLE_BODY_TD_CLASS}>
