...
// Records one partial (or final) payment against a single invoice. Can be
// called repeatedly while a balance remains — the backend rejects amounts
// over the remaining balance and flips status to "Paid" once it's cleared.
export function recordInvoicePayment(
  id: string,
  payload: { amount: number; method?: string; note?: string }
) {
  return request<{ invoice: any; message: string }>(`/${id}/payments`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
