...
type InvoiceStatus = "Draft" | "Sent" | "PartiallyPaid" | "Paid" | "Overdue";
...
/** Once any payment is recorded, an invoice can no longer be edited, archived, or deleted. */
function isInvoiceLocked(row: Pick<InvoiceRow, "amountPaid">) {
  return (row.amountPaid || 0) > 0;
}
function balanceDue(row: Pick<InvoiceRow, "invoiceTotal" | "amountPaid">) {
  return Math.max(0, round2(row.invoiceTotal - (row.amountPaid || 0)));
}
...
export default function InvoicesPage() {
  ...
  // ── Enter Payment modal (partial or final payment against ONE invoice) ──
  ...
  const openEdit = async (row: InvoiceRow) => {
    if (isInvoiceLocked(row)) {
      adminNotify.error(
        row.status === "Paid"
          ? "This invoice is fully paid and can no longer be edited."
          : "This invoice has a payment recorded against it and can no longer be edited."
      );
      return;
    }
    await loadReferenceData();
  ...
  const runBulk = async (
    action: "archive" | "delete" | "restore" | "send" | "markDraft",
    successMsg: string
  ) => {
    if (selected.size === 0) return;
    try {
  ...
  const selectedRows = () => invoices.filter((row) => selected.has(row._id));

  const handleArchive = () => {
    const locked = selectedRows().filter(isInvoiceLocked);
    if (locked.length > 0) {
      adminNotify.error(
        `${locked.length} of the selected invoice(s) have a payment recorded and can't be archived.`
      );
      return;
    }
    runBulk("archive", "Invoice(s) archived.");
  };
  const handleDelete = () => {
    const locked = selectedRows().filter(isInvoiceLocked);
    if (locked.length > 0) {
      adminNotify.error(
        `${locked.length} of the selected invoice(s) have a payment recorded and can't be deleted.`
      );
      return;
    }
    runBulk("delete", "Invoice(s) deleted. Stock has been restored.");
  };
  const handleMarkDraft = () => runBulk("markDraft", "Marked as Draft.");
  const handleSend = () => runBulk("send", "Invoice(s) sent.");
  const handleRestore = () => runBulk("restore", "Invoice(s) restored.");

  // "Enter Payment" now opens a modal for ONE invoice at a time — partial
  // payments need an amount, so a blanket bulk "mark all paid" no longer
  // makes sense once invoices can be paid in installments.
  const handleEnterPayment = () => {
    if (selected.size !== 1) {
      adminNotify.error("Select exactly one invoice to enter a payment for.");
      return;
    }
    const row = selectedRows()[0];
    if (!row) return;
    if (row.status === "Paid") {
      adminNotify.error("This invoice is already fully paid.");
      return;
    }
    setPaymentRow(row);
    setPaymentAmount(String(balanceDue(row)));
    setPaymentMethod("");
    setPaymentNote("");
  };
  ...
  const submitPayment = async () => {
    ...
    const remaining = balanceDue(paymentRow);
    if (!Number.isFinite(amount) || amount <= 0) {
      adminNotify.error("Enter a valid payment amount greater than 0.");
      return;
    }
    if (amount > remaining + 0.01) {
      adminNotify.error(`Payment can't exceed the remaining balance of ${fmtMoney(remaining)}.`);
      return;
    }
    setRecordingPayment(true);
    try {
      const res = await recordInvoicePayment(paymentRow._id, {
        amount,
        method: paymentMethod.trim() || undefined,
        note: paymentNote.trim() || undefined,
      });
      adminNotify.success(res.message || "Payment recorded.");
      closePaymentModal();
      setSelected(new Set());
      loadInvoices();
    } catch (err: any) {
      adminNotify.error(err.message || "Failed to record payment.");
    } finally {
                  ...
                  paged.map((row, idx) => (
                    <tr key={row._id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-100"}>
                      ...
                      <td className="border border-gray-300 px-3 py-2 text-left">
                        ...
                        {isInvoiceLocked(row) && row.status !== "Paid" && (
                          <div className="text-[11px] text-gray-500">
                            {fmtMoney(row.amountPaid || 0)} paid · {fmtMoney(balanceDue(row))} due
                          </div>
                        )}
                      </td>
                      <td className="border border-gray-300 px-3 py-2 text-left">
                        <button
                          type="button"
                          onClick={() => openEdit(row)}
                          disabled={isInvoiceLocked(row)}
                          title={isInvoiceLocked(row) ? "Locked — a payment has been recorded" : "Edit"}
                          className="text-blue-700 hover:text-blue-900 disabled:cursor-not-allowed disabled:text-gray-400"
                          aria-label={`Edit invoice ${row.invoiceNumber}`}
                        >
                          {isInvoiceLocked(row) ? "🔒" : "✎"}
                        </button>
        ...
        >
          <div className="w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <CompactFormPanel
              footer={
                <CompactFormFooter
                  actionLabel={recordingPayment ? "Recording..." : "Record Payment"}
            ...
            >
              <div className="mb-3 flex items-center gap-1.5 text-sm font-bold text-ad-green-dark">
                <span aria-hidden>💵</span> Enter Payment — {paymentRow.invoiceNumber}
              </div>
              <div className="space-y-4">
                <div className="rounded bg-gray-100 px-3 py-2 text-xs text-gray-700">
                  <div>Invoice total: <span className="font-semibold">{fmtMoney(paymentRow.invoiceTotal)}</span></div>
                  <div>Already paid: <span className="font-semibold">{fmtMoney(paymentRow.amountPaid || 0)}</span></div>
                  <div>Remaining balance: <span className="font-semibold">{fmtMoney(balanceDue(paymentRow))}</span></div>
                </div>
                <CompactField label="Payment Amount" required>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                    max={balanceDue(paymentRow)}
                    value={paymentAmount}
                ...
                <p className="text-[11px] text-gray-500">
                  A partial payment leaves this invoice as <span className="font-semibold">Partially Paid</span> and
                  locked against edits — paying it in full marks it <span className="font-semibold">Paid</span>.
                  Neither state can be edited, archived, or deleted afterwards.
                </p>
              </div>
            </CompactFormPanel>
          </div>
