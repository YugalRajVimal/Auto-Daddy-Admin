...
// ): Promise<TemplateRow[]> {
//   if (USE_SAMPLE_DATA) {
//     // Filter sample data by country and userType (simulate server-side filtering)
//     const checkedUserTypes = Object.entries(userTypeFilters)
//       .filter(([, checked]) => checked)
//       .map(([type]) => type);
//     return SAMPLE_TEMPLATES.filter(
//       (tpl) => tpl.country === country && checkedUserTypes.includes(tpl.userType)
//     );
//   }
...
//     apiFetchTemplates(country, userTypeFilters).then((data) => {
...
//         <b>Sample Data Mode:</b> This page displays <b>demo/sample data</b> – no real backend communication occurs.
//       </div> */}
//       {isDeletedView && (
...
export default function InvoiceTemplatesPage({ initialShowForm = false }: InvoiceTemplatesPageProps) {
  ...
  const handleToolbarPrint = () => {
    printAdminTable({
      title: "Invoice Templates",
      headers: ["Template", "Design", "Date", "Country", "User Type", "Used by", "Clip"],
      rows: filtered.map((template) => [
        template.notes,
