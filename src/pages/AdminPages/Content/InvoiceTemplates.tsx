...
//   { value: "tire-master", label: "Tyre Shop" },
//   { value: "car-washing", label: "Car Wash" },
...
// Full-page modal preview shown on click of the eye icon
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
          <div>
            <p className="text-sm font-semibold text-gray-800">{row.notes}</p>
            <p className="text-xs text-gray-500">
              {getTemplateLabel(row.templateSlug)} &middot; {getUserTypeLabel(row.userType)}
            </p>
          </div>
          <button
            type="button"
            ...
            style={{ padding: 0, background: "none", border: "none", lineHeight: 1, cursor: "pointer" }}
          >
...
export default function InvoiceTemplatesPage({ initialShowForm = false }: InvoiceTemplatesPageProps) {
  ...
  // For preview popover: control which ID is hovered
  const [hoveredPreviewId, setHoveredPreviewId] = useState<string | null>(null);
  // For full-page modal preview: control which ID is open in the modal
  ...
  const modalRow = modalPreviewId
    ? displayTemplates.find((t) => t.id === modalPreviewId) ?? null
    : null;
                    ...
                    <div className="">
                      ...
                      >
                        <button
                          ...
                          onFocus={() => setHoveredPreviewId(row.id)}
                          onBlur={() => setHoveredPreviewId(null)}
                          onClick={() => setModalPreviewId(row.id)}
                          style={{ padding: 0, background: "none", border: "none", lineHeight: 1, cursor: "pointer" }}
                        >
                          <svg width="18" height="18" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M10 4.5c-5 0-8 4.1-8 5.5s3 5.5 8 5.5 8-4.1 8-5.5-3-5.5-8-5.5zm0 9A4 4 0 1 1 10 7a4 4 0 0 1 0 8zm0-2.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
