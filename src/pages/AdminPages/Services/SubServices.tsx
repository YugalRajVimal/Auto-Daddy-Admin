...
// === Helper to get admin-token for headers ===
...
export default function SubServicesPage({ initialShowForm = false }: SubServicesPageProps) {
  ...
  const resetForm = () => {
    reset({ serviceId: "", name: "", status: "active" });
    setEditingRow(null);
    setError("");
  };
    ...
    async (values) => {
      ...
      try {
        ...
        if (editingRow) {
          updated = existing.map((s) =>
            s.name === editingRow.name
              ? {
                  ...s,
                  name: values.name,
                  status: values.status as SubServiceStatus,
          ...
          updated = [
            ...existing,
            { name: values.name, status: values.status as SubServiceStatus, createdBy: "admin" },
          ];
        }
        await axios.put(
  ...
  const handleToolbarDelete = async () => {
    ...
    const { succeeded, failed, firstError } = await runBulk(groupByParent(list), async ({ categoryId, children }) => {
      ...
      await axios.put(`${API_BASE}/admin/services/${categoryId}`, { subServices: updated }, { headers: getAuthHeaders() });
    });
  ...
  const handleRestore = async () => {
    const list = sortedDeletedStash.filter((row) => selected.has(getRowId(row)));
    if (list.length === 0) return;
    const what = list.length === 1 ? `sub service "${list[0].name}"` : countLabel(list.length, "sub service");
    ...
    const { succeeded, failed, firstError } = await runBulk(groupByParent(list), async ({ categoryId, children }) => {
      ...
      const existing: SubService[] = (parent.subServices || []).map((s) =>
        mapApiSubService(s as Record<string, unknown>));
      const updated = [...existing, ...children.map((row) => ({
        name: row.name,
