...
type AutoShopOwnerType = {
  ...
  onboardedByStaffName?: string;
  onboardedByStaffRole?: string;
};
...
const OnboardingStatusBadge: React.FC<{ status?: string }> = ({ status }) => {
  const value = status || "pending";
  const label = value.charAt(0).toUpperCase() + value.slice(1);
  return (
    <span
      className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold ${
        ONBOARDING_STATUS_STYLES[value] || ONBOARDING_STATUS_STYLES.pending
      }`}
    >
      {label}
    </span>
  ...
  function renderCell(owner: AutoShopOwnerType, key: string) {
    switch (key) {
      ...
      case "onboardingStatus":
        return (
          <td key={key} className={tdClass}>
            <OnboardingStatusBadge status={owner.onboardingStatus} />
            {owner.onboardedByStaffName && (
              <div className="text-[11px] text-gray-500 mt-0.5">by {owner.onboardedByStaffName}</div>
            )}
          </td>
        );
      default:
