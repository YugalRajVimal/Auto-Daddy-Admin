...
//   updatedAt?: string;
//   __v?: number;
//   // Additional property: rating, reviewCount, reviewDate, website, directions, city, services if any, etc.
//   city?: string;
...
//   // Other dynamic props can exist (but not services!), so extend as index signature
//   [key: string]: any;
// };
...
// // Helper to prettify business details for the ShopOverviewCard
// function toShopOverviewProps(shop: BusinessProfileType) {
//   // Fallbacks for mock data if no real
...
//       : "#");
//   // Optional: services can be real myServices or fallback like in design
//   let services: string[] = [];
...
//     ];
//   }
//   // Is open based on time/day info; fallback yes
//   const isOpen = typeof shop.openHours === "string" && typeof shop.openDays !== "undefined"
//     ? true
//     : true; // Use true by default
//   return {
//     phone,
//     businessName,
//     address,
...
// // --- Card for Shop Overview (uses reference design above) ---
...
//   return (
//     <div className="w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_4px_24px_rgba(15,23,42,0.06)]">
//       {/* Top action bar */}
//       <div
...
// // --- Favorite AutoShops Modal Content (Card style) ---
// const renderFavShopsModalContent = (owner: CarOwnerType) => (
//   <>
//     {/* Onboarded By and Owner Email Section */}
//     <div className="mb-6">
...
// // --- Jobs Modal & Table, Vehicles Modal: remain unchanged (show job cards/vehicles as before) ---
...
// // --- UNCHANGED: Job Cards Modal & content (reuse as in original) ---
...
//   const UPLOADS_URL = import.meta.env.VITE_UPLOADS_URL;
//   // We purposely do NOT destructure 'services' from card because it doesn't exist in JobCardTypePopulated's type.
//   // Instead, use (card as any).services safely for optional reading.
...
//                   <>
//                     <span>
//                       Created: {new Date((card as any).createdAt).toLocaleString()}
//                     </span>
//                   </>
//                 )}
...
//                   <>
//                     <span>
//                       Updated: {new Date((card as any).updatedAt).toLocaleString()}
//                     </span>
//                   </>
//                 )}
//               </div>
...
//                 <div><span className="font-medium">Plate No:</span> {(card as any).vehicleId?.licensePlateNo || "-"}</div>
//               </div>
...
function toShopOverviewProps(shop: BusinessProfileType) {
  ...
  if (shop.myServices && Array.isArray(shop.myServices) && shop.myServices.length > 0) {
    services = shop.myServices.map((s: any) =>
      typeof s === "string" ? s : typeof s?.name === "string" ? s.name : "Unknown Service"
    );
  } else {
    services = [
      "General Repair",
  ...
  // PATCH: Show isFav exactly as received on shop
...
function ShopOverviewCard(shop: BusinessProfileType) {
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_4px_24px_rgba(15,23,42,0.06)] relative">
      {/* ❤️ Favourite heart — top-right corner */}
      <div
        className="absolute top-3 right-3 z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm shadow-sm border border-slate-100"
        title={isFav ? "Marked as favourite" : "Not a favourite"}
      >
          ...
          <div className="max-w-full overflow-x-auto">
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                ...
                {carOwners.map((owner) => {
                  const servicedShops = getAutoshopsReceivedServiceFrom(owner);
                  return (
                    <TableRow key={owner._id}>
                      <TableCell className="px-5 py-3 text-gray-800 text-theme-sm dark:text-white/90">
                        {owner.name || "-"}
