...
// Local (not UTC) yyyy-mm-dd for today — used as the ledger form's default
// date so new entries default to "today" instead of a stale hardcoded date.
...
function buildLedgerSearchFields(variant: "expenses" | "income"): AdminSearchField[] {  const fields: AdminSearchField[] = [
    { key: "date", label: "Date", type: "date" },
    { key: "vendor", label: "Vendor" },
    { key: "amount", label: "Amount" },
  ];
...
function LedgerPage({
  ...
  // Total column count for this variant/view — keeps the loading/empty-state
  // row's colSpan in sync with however many <th> are actually rendered
