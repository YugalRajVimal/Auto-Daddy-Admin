...
import { getHomePathForRole, getPortalForRole, getSignInPathForRole } from "./roleRegistry";
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // Validate admin-portal session on mount
  useEffect(() => {
    let cancelled = false;
    ...
    async function validate() {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/auth/admin/check-auth/`, {
          ...
          body: JSON.stringify({ role: current.role === "subadmin" ? "subadmin" : "admin" }),
        });
        ...
        if (!cancelled) {
          // Keep session on network error — layout will still work offline briefly
          setSession(current);
          setIsLoading(false);
        }
      }
