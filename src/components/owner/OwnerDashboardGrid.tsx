...
import OwnerShopAdsPanel from "./OwnerShopAdsPanel";
...
function StatCard({
  ...
  return (
    ...
    >
      <div
        className={`pointer-events-none absolute -right-6 -top-6 size-20 rounded-full ${accent.soft} opacity-80 transition group-hover:scale-110 md:size-24`}
      />
      <div className="relative flex items-start justify-between gap-2 md:gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-500 md:text-[11px]">
            {label}
          </p>
          {loading ? (
            <Skeleton className="mt-1.5 h-7 w-12 rounded md:mt-2 md:h-8 md:w-14" />
          ) : (
            <p className="mt-0.5 text-xl font-bold tabular-nums tracking-tight text-slate-900 md:text-2xl 2xl:text-3xl">
              {value}
            </p>
          )}
        </div>
      ...
      <div className="relative mt-2 flex items-center justify-between gap-2 md:mt-2.5 2xl:mt-3">
        ...
        <span
          className={`inline-flex items-center gap-0.5 text-xs font-semibold ${accent.text} opacity-0 transition group-hover:opacity-100`}
        >
...
function OdometerQuickUpdate({
  ...
  const current =
    selected?.odometerReading != null
      ? String(selected.odometerReading).trim()
      : "";
  ...
  const readingForRemaining =
    parsed != null && Number.isFinite(parsed) ? parsed : currentNum;
  const remaining = remainingKmNumber(dueNum, readingForRemaining);
  const progressPct =
    dueNum != null && dueNum > 0 && readingForRemaining != null
      ? Math.max(
          0,
          Math.min(100, Math.round((readingForRemaining / dueNum) * 100)),
        )
      : 0;
              ...
              <label className="block text-xs font-semibold text-slate-600">
                ...
                >
                  {vehicles.map((v, index) => {
                    ...
                    const make = [v.make?.name, v.make?.model]
                      .filter(Boolean)
                      .join(" ");
                    return (
...
export default function OwnerDashboardGrid() {
  ...
  const { sections: docSections, loading: docsLoading } =
    useCarOwnerDocuments();
  const { favoriteIds, loading: favoritesLoading } = useCarOwnerFavoriteShops();
  const { shops, loading: shopsLoading } = useCarOwnerAutoShops({
    serviceIds: [],
  });
  const {
    indoor,
  ...
  const documentsUploaded = useMemo(
    () =>
      docSections.reduce(
        (count, section) =>
          count + section.fields.filter((f) => Boolean(f.uri)).length,
        0,
      ),
    [docSections],
  );
  ...
  const shopTypeEntries = (
    Object.keys(SHOP_TYPE_LABELS) as OwnerShopType[]
  ).map((key) => ({
    key,
  ...
  const serviceDeals =
    grouped.Service.city.length + grouped.Service.others.length;
  ...
  return (
    <div className="grid grid-cols-1 gap-2.5 md:grid-cols-[160px_minmax(0,1fr)] lg:grid-cols-[180px_minmax(0,1fr)] xl:grid-cols-[200px_minmax(0,1fr)] 2xl:gap-3">
      ...
      <div className="space-y-3 2xl:space-y-4">
        <div
          className={`${ownerPageIntroClass} flex flex-wrap items-end justify-between gap-2 2xl:gap-3`}
        >
          <div>
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 md:grid-cols-4 md:gap-2.5 2xl:gap-3">
          ...
          <StatCard
            ...
            progress={
              invoiceRows.length
                ? Math.round(
                    (paidInvoices.length / Math.max(invoiceRows.length, 1)) *
                      100,
                  )
                : 8
            }
          ...
          <StatCard
            ...
            accent={ACCENTS[6]}
            progress={
              favoriteIds.size ? Math.min(100, favoriteIds.size * 25) : 8
            }
          />
          <StatCard
            to="/owner/auto-shops"
        ...
        <div className="grid grid-cols-1 gap-2.5 md:grid-cols-3 2xl:gap-3">
          <div className="rounded-2xl border border-white/80 bg-white/95 p-3 shadow-[0_8px_24px_rgba(15,23,42,0.06)] ring-1 ring-sky-100 2xl:p-4 md:col-span-2">
            ...
            {servicesLoading ? (
              ...
              <p className="mt-4 text-sm text-slate-600">
                Shop categories will appear once services load.
              </p>
            ) : (
              <div className="mt-2">
                <Chart
                  ...
                  series={[
                    {
                      name: "Count",
                      data: shopTypeEntries.map((e) => e.count),
                    },
                  ]}
                  type="bar"
                  height={220}
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                >
                  <FiHeart
                    size={16}
                    className={
                      thoughtOfTheDayLiked ? "fill-current" : undefined
                    }
                  />
                </button>
              </div>
            ...
            <div className="flex-1 rounded-2xl border border-white/80 bg-white/95 p-3 shadow-[0_8px_24px_rgba(15,23,42,0.06)] ring-1 ring-orange-100 2xl:p-4">
              ...
              {dashboardLoading ? (
                ...
                <p className="mt-3 text-sm text-slate-600">
                  No upcoming service due right now.
                </p>
              )}
            </div>
          </div>
