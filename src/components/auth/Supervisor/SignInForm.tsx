...
export default function SupervisorSignInForm() {
  ...
  const handleGetOTP = async (values: SignInEmailValues) => {
    ...
    try {
      const res = await axios.post(
        ...
        { email: values.email, role: "Supervisor" }
      );
  ...
  const handleLogIn = async (values: SignInOtpValues) => {
    ...
    try {
      const res = await axios.post(
        ...
        { email, otp: values.otp, role: "Supervisor" }
      );
      ...
      if (res.data.token) {
        localStorage.setItem("supervisor-token", res.data.token);
      }
