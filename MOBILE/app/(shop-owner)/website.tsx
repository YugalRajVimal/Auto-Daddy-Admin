import { CashfreeCheckoutModal } from "@/components/shop-owner/cashfree-checkout-modal";
...
import { useCashfreePaymentCallback } from "@/hooks/use-cashfree-payment-callback";
import {
...
import {
  extractPurchaseSubscriptionCashfreeSession,
  fetchWebsiteTemplates,
  extractPendingSubscriptionCheckout,
  extractPurchaseSubscriptionPaymentLink,
...
export default function WebsitePage() {
  ...
  const confirmCashfreePayment = useCallback(
    async (orderId: string) => {
      ...
      showToast(
        "Payment not confirmed yet. Complete payment in Cashfree, then pull to refresh. Your shop backend must receive the Cashfree webhook to mark the order Paid.",
        { type: "info" }
      );
    },
    [isOwner, showToast, token]
  ...
  useCashfreePaymentCallback({
    onVerify: (orderId) => {
      void confirmCashfreePayment(orderId);
    },
    onError: (error, orderId) => {
      const detail = error.getMessage()?.trim();
      showToast(
        detail
  ...
  const openCashfreeCheckout = useCallback(
    (paymentUrl: string, session?: { paymentSessionId: string; orderId: string } | null) => {
      if (!session?.paymentSessionId) {
        showToast("Payment session not returned.", { type: "error" });
        return;
      }
      const preferNative =
        process.env.EXPO_PUBLIC_CASHFREE_USE_NATIVE?.trim().toLowerCase() === "true";
      if (preferNative && isCashfreeNativeAvailable()) {
        try {
          startCashfreeWebCheckout(session, {
            paymentLink: isCashfreeMerchantApiUrl(paymentUrl) ? null : paymentUrl,
          });
          return;
        } catch {
          // Fall through to in-app WebView checkout.
        }
      }
      setCheckout({
        paymentSessionId: session.paymentSessionId,
        orderId: session.orderId,
        paymentUrl: isCashfreeMerchantApiUrl(paymentUrl) ? "" : paymentUrl,
      });
    },
    [showToast]
  );
  ...
  const handleMakePayment = useCallback(async () => {
    ...
    if (pending?.session.paymentSessionId) {
      showToast("Opening pending payment…", { type: "info" });
      openCashfreeCheckout(pending.paymentUrl, pending.session);
      return;
    }
    ...
    try {
      ...
      const cashfreeSession = extractPurchaseSubscriptionCashfreeSession(data);
      const paymentLink = extractPurchaseSubscriptionPaymentLink(data) ?? "";
      if (!cashfreeSession) {
      ...
      openCashfreeCheckout(paymentLink, cashfreeSession);
    } catch {
      showToast("Network error starting payment.", { type: "error" });
    } finally {
      setPaymentLoading(false);
    }
  }, [openCashfreeCheckout, profileData?.businessProfile, showToast, token]);
