import type { AuthSessionMeta } from "@/lib/auth-meta";
import { digitsFromNationalPhoneDisplay } from "@/lib/national-phone-format";
import type {
  AutoShopOwnerBusinessProfile,
  AutoShopOwnerUserProfile,
} from "@/types/auto-shop-owner-profile";

function phoneDigits(value: string): string {
  return digitsFromNationalPhoneDisplay(value);
}

export function hasEstablishedPersonalProfile(
  user?: AutoShopOwnerUserProfile | null,
  city?: string | null
): boolean {
...
export function hasEstablishedBusinessProfile(
  business?: AutoShopOwnerBusinessProfile | null,
  zipCode?: string | null
): boolean {
  ...
  return Boolean(
    business?.businessName?.trim() &&
      ...
      business?.city?.trim() &&
      phone.length >= 10 &&
      (zipCode?.trim() || business?.pincode?.trim() || business?.businessEmail?.trim())
  );
}

/** `true` incomplete, `false` complete, `null` unknown (still loading). */
export function resolvePersonalProfileIncomplete(
  meta: Pick<AuthSessionMeta, "isProfileComplete"> | null | undefined,
  user: AutoShopOwnerUserProfile | null | undefined,
...
export function resolveBusinessProfileIncomplete(
  meta: Pick<AuthSessionMeta, "isAutoShopBusinessProfileComplete"> | null | undefined,
  business: AutoShopOwnerBusinessProfile | null | undefined,
...
/** Forced completion target — Expo business setup screen. */
