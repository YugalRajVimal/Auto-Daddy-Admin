...
/** POST /api/auto-shop-owner/purchase-subscription — Stripe / Cashfree website subscription checkout */
...
export function extractPurchaseSubscriptionPaymentLink(
  ...
  if (stripeUrl && isStripeHostedCheckoutUrl(stripeUrl)) return stripeUrl;
...
export function extractPurchaseSubscriptionCheckoutSession(
  ...
  const stripeSessionId =
    ...
    (payload.sessionId?.trim() && isStripeCheckoutSessionId(payload.sessionId)
  ...
  if (checkoutUrlRaw) {
    if (isStripeHostedCheckoutUrl(checkoutUrlRaw)) {
      checkoutUrl = checkoutUrlRaw;
    } else if (!isCashfreeMerchantApiUrl(checkoutUrlRaw)) {
      checkoutUrl = checkoutUrlRaw;
    }
  }

  const paymentLink = payload.paymentLink?.trim();
  if (!checkoutUrl && paymentLink && isStripeHostedCheckoutUrl(paymentLink)) {
  ...
  if (stripeSessionId || checkoutUrl || clientSecret) {
    return {
      orderId,
...
function isStripeCheckoutSessionId(value: string): boolean {
  return /^cs_/i.test(value.trim());
}

function isCashfreePaymentSessionId(value: string): boolean {
  const id = value.trim();
  return id.startsWith("session_") && !isStripeCheckoutSessionId(id);
}
...
export function extractPurchaseSubscriptionCashfreeSession(
  ...
  if (!isCashfreePaymentSessionId(paymentSessionId)) return null;
...
/** Pending Stripe subscription on profile — resume checkout without a new order. */
export function extractPendingSubscriptionCheckout(
  ...
  for (const sub of subs) {
    ...
    const paymentMethod = (sub.paymentMethod ?? "").toLowerCase();
    if (paymentMethod === "cashfree") continue;
