...
import { formatNationalPhoneDisplay } from "@/lib/national-phone-format";
import { getWalletLedgerTab, isOnlineInvoicePayment } from "@/lib/wallet-helpers";
...
export function formatEstimateMoney(
  ...
  return formatCurrencyNumber(amount, countryCode) ?? "—";
}
...
export function pickBusinessHstNumber(
  ...
  const profile = business as
    | (ShopProfileBusiness & { businessHSTNumber?: string; gstNumber?: string })
    | null;
  const fromProfile =
  ...
  const embedded =
    nested(job?.business) ?? nested(job?.businessProfile) ?? nested(job?.shop);
  if (embedded) {
    return (
...
export function buildBusinessBlock(business: ShopProfileBusiness | null | undefined) {
  ...
  const address = [
    s(business?.address) || s(business?.businessAddress),
    s(business?.city),
  ]
    .filter(Boolean)
    .join(", ");
  const phoneRaw = s(business?.businessPhone);
  const phone = phoneRaw ? formatNationalPhoneDisplay(phoneRaw) : "";
