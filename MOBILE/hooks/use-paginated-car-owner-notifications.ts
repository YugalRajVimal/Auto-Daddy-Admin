...
export const CAR_OWNER_NOTIFICATIONS_DEFAULT_LIMIT = 20;
...
export function usePaginatedCarOwnerNotifications(
  authToken: string | null,
  enabled: boolean,
  limit = CAR_OWNER_NOTIFICATIONS_DEFAULT_LIMIT
) {
  ...
  const runFetch = useCallback(
    async (pageNum: number, mode: "replace" | "append"): Promise<CarOwnerNotification[] | undefined> => {
      ...
      try {
        ...
        const parsed = parseCarOwnerNotificationsResponse(res.data, limit);
