...
function actionError(res: { ok: boolean; data: unknown }, fallback: string): string {
  if (res.data && typeof res.data === "object" && "message" in res.data) {
    const msg = String((res.data as { message?: unknown }).message ?? "").trim();
    if (msg) return msg;
  }
  return fallback;
}
...
export function useCarOwnerJobCards(vehicleId?: string | null) {
  ...
  const normalizeJobCardsPayload = useCallback(
    (payload: CarOwnerJobCardsBuckets | undefined): CarOwnerJobCard[] => {
      if (!payload) return [];
  ...
  const load = useCallback(async () => {
    ...
    const res = await getJson<CarOwnerJobCardsResponse>(jobCardsPath(vehicleFilter), {
      authToken: token,
    });
    ...
    const next = normalizeJobCardsPayload(resolveJobCardsBuckets(res.data ?? undefined));
  ...
  const approveJobCard = useCallback(
    async (jobCardId: string): Promise<{ ok: boolean; error?: string }> => {
      ...
      if (!res.ok) {
        return { ok: false, error: actionError(res, "Could not approve job card.") };
      }
      if (
        res.data &&
  ...
  const rejectJobCard = useCallback(
    async (jobCardId: string): Promise<{ ok: boolean; error?: string }> => {
      ...
      if (!res.ok) {
        return { ok: false, error: actionError(res, "Could not reject job card.") };
      }
      if (
        res.data &&
        typeof res.data === "object" &&
