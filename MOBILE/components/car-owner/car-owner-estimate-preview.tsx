import { A4_PAGE_PAD_PX, A4DocumentSheet } from "@/components/job-cards";
...
/**
 * Car-owner Expenses preview — A4 sheet (same as shop-owner estimate viewer):
 * - jobcard: blue viewer theme
 * - invoice: magenta modern invoice
 */
export function CarOwnerEstimatePreview({
  ...
  const docNo = useMemo(() => {
    if (isInvoice) {
      ...
      return (job ? resolveJobCardNo(job) : "") || "—";
    }
    if (!job) return "—";
    return resolveJobCardNo(job) || "—";
  ...
  const documentHeading = isInvoice ? "Invoice" : "Job Card";
  const documentNoLabel = isInvoice ? "Invoice No. :" : "Job Card No. :";
        ...
        <View style={styles.docBody}>
          ...
          {!isInvoice && vehicle ? (
            <View style={styles.vehicleBlock}>
              ...
              <Text style={styles.toName}>{vehicle.plate}</Text>
